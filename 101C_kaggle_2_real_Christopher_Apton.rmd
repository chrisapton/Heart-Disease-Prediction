...
boost_model3 <- boost_tree(mode = "classification") %>% 
  set_engine("xgboost")
...
model_list = list(boost_model_1 = boost_model, boost_model_2 = boost_model2, boost_model_3 = boost_model3, svm_model = svm_model, knn_spec = knn_spec)
```
...
# tuning_grid <- grid_regular(trees(range(100,500)), 
# learn_rate(range(-1,-0.4)), levels = 15)

# wf <- workflow() %>%
#   add_model(boost_model2) %>%
#   add_formula(num ~ . )

# res <- tune_grid(wf, resamples = ames_folds, grid = tuning_grid)
```


```{r}
 # tuning 2
# res %>% collect_metrics() %>% filter(.metric == "accuracy") %>% 
# arrange(desc(mean))
```
